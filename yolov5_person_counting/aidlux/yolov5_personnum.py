from cvs import *
import aidlite_gpu
from utils import detect_postprocess, preprocess_img, draw_detect_res, process_points, is_passing_line
import cv2# bytetrack
...
track_id_status = {}
cap = cvs.VideoCapture(r"/home/lesson3_aidlux/aidlux/video.mp4")
frame_id = 0
count_person=0
while True:    
    frame = cap.read()    
    ...
    for box in pred[0]:  # per image        
        ...
        if len(det):        
            ...
            for t in online_targets:            
                ...
                ### 越界识别功能实现 ###            
                # 1.绘制越界监测区域            
                line_points = [[186, 249], [1235, 366]]            
                color_green = (0, 255, 0)  ##绿色    
                cv2.line(res_img, (186,249), (1235,366), color_green, 3)        
                ...
                track_info = is_passing_line(pt, line_points)            
                ...
                if track_id_status[tid][-1] == 1 and len(track_id_status[tid]) >1:          
                    ...
                    if  track_id_status[tid][-2] == -1:                    
                        ...
                        count_person += 1    
    cv2.putText(res_img, "person_count:"+str(count_person), (100,600 ), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,255), 2)   
    cvs.imshow(res_img)